...
class ImageSignature(object):
    ...
    def compute_grid_points(image, n=9, window=None):
        ...
        x_coords = np.linspace(window[0][0], window[0][1], n + 2)[1:-1]
        y_coords = np.linspace(window[1][0], window[1][1], n + 2)[1:-1]
    ...
    def compute_mean_level(image, x_coords, y_coords, P=None):
        ...
        for i, x in enumerate(x_coords):        # not the fastest implementation
            ...
            for j, y in enumerate(y_coords):
                ...
                #print upper_y_lim
                #print "==="
                avg_grey[i, j] = np.mean(image[int(lower_x_lim):int(upper_x_lim),
                                        int(lower_y_lim):int(upper_y_lim)])  # no smoothing here as in the paper
